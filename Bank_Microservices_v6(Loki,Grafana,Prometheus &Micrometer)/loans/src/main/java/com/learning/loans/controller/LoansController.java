...
public class LoansController {
    ...
    @Operation(
            ...
            description = "REST API to create new loan inside RamBank"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    @GetMapping("/fetchLoanBymobileNumber")
    public ResponseEntity<LoansDto> fetchLoanDetails(@RequestHeader("ramBank-correlation-id") String correlationId,@RequestParam
                                                     @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile number must be 10 digits")
                                                     String mobileNumber) {
